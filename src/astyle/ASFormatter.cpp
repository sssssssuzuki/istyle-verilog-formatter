...
string ASFormatter::nextLine() {
  ...
  while (!isLineReady) {
    ...
    if (currentChar == PREPROCESSOR_CHAR && (findHeader(preprocessorHeaders) != NULL)) isInPreprocessor = true;
    ...
    if (isImmediatelyPostHeader) {
      ...
      if (shouldBreakOneLineStatements) {
        ...
        if (shouldBreakElseIfs)
          ...
          if (currentHeader == &AS_ELSE && upcomingHeader == &AS_IF) isInElseIf = true;

          if (!isInElseIf) isInLineBreak = true;  ////BUGFIX: SHOULD NOT BE breakLine() !!!
          ...
          if (shouldPadBlocks && !isInElseIf) {
            ...
            if (vBlockBegin == NULL &&
                (currentHeader == &AS_IF || currentHeader == &AS_ELSE || currentHeader == &AS_FOR || currentHeader == &AS_WHILE)) {
              currentChar = 'b';
              string str1 = "begin ";
    ...
    if (bracketFormatMode != NONE_MODE) {
      if (currentChar == '{') {
        if (bracketFormatMode == ATTACH_MODE && !isCharImmediatelyPostLineComment) {
          appendSpacePad();
    ...
    if ((newHeader = findHeader(headers)) != NULL) {
      ...
      isNonParenHeader = (find(nonParenHeaders.begin(), nonParenHeaders.end(), newHeader) != nonParenHeaders.end());
      ...
      if (currentHeader == &AS_IF && previousHeader == &AS_ELSE) isInLineBreak = false;
      ...
      if (shouldBreakBlocks) {
        ...
        if (currentHeader == &AS_ELSE || isCharImmediatelyPostLineComment || isCharImmediatelyPostComment) {
          isPrependPostBlockEmptyLineRequested = false;
        }
    ...
    if (previousNonWSChar == '}' || currentChar == ';') {
      if (shouldBreakOneLineStatements && currentChar == ';' && (shouldBreakOneLineBlocks)) {
    ...
    if (currentChar == ':') {
      if (previousNonWSChar == '}' || previousNonWSChar == '{') {
        ...
        continue;
      } else if (shouldBreakOneLineStatements && previousNonWSChar != '}' && previousNonWSChar != '{' &&
                 !foundQuestionMark  // not in a ... ? ... : ... sequence
                 && previousCommandChar !=
                        ')'  // not immediately after closing paren of a method header, e.g. ASFormatter::ASFormatter(...) : ASBeautifier(...)
      ) {
        passedColon = true;
      }
    }
    ...
    if (shouldPadOperators) {
      if ((newHeader = findHeader(operators)) != NULL) {
        bool shouldPad = (newHeader != &AS_PAREN_PAREN && newHeader != &AS_BLPAREN_BLPAREN && newHeader != &AS_NOT && newHeader != &AS_BIT_NOT &&
                          newHeader != &AS_BITNOT_AND && newHeader != &AS_BITNOT_OR && newHeader != &AS_NOT_XNOR && newHeader != &AS_BITNOT_XNOR &&
                          !(newHeader == &AS_MINUS && isInExponent()) && !(newHeader == &AS_PLUS && isInExponent()) &&
                          !((isCharImmediatelyPostTemplate) && (newHeader == &AS_LS || newHeader == &AS_GR)));

        // pad before operator
        if (shouldPad && !(newHeader == &AS_COLON && !foundQuestionMark) && newHeader != &AS_SEMICOLON && newHeader != &AS_COMMA) appendSpacePad();
        appendSequence(*newHeader);
        goForward(newHeader->length() - 1);

        // since this block handles '()' and '[]',
        // the parenStack must be updated here accordingly!
        if (newHeader == &AS_PAREN_PAREN || newHeader == &AS_BLPAREN_BLPAREN) parenStack->back()--;

        currentChar = (*newHeader)[newHeader->length() - 1];
        // pad after operator
        // but do not pad after a '-' that is a urinary-minus.
        if (shouldPad && !(newHeader == &AS_MINUS && isUrinaryMinus())) appendSpacePad();
    ...
    if (shouldPadParenthesies) {
      if (currentChar == '(' || currentChar == '[') {
        ...
        appendCurrentChar();
        if (!(currentChar == '(' && peekedChar == ')') && !(currentChar == '[' && peekedChar == ']')) appendSpacePad();
        continue;
      } else if (currentChar == ')' || currentChar == ']') {
        char peekedChar = peekNextChar();

        if (!(previousChar == '(' && currentChar == ')') && !(previousChar == '[' && currentChar == ']')) appendSpacePad();

        appendCurrentChar();

        if (peekedChar != ';' && peekedChar != ',' && peekedChar != '.' && !(currentChar == ']' && peekedChar == '[')) appendSpacePad();
        continue;
      }
    }
  ...
  if (prependEmptyLine && readyFormattedLineLength > 0 && previousReadyFormattedLineLength > 0) {
 ...
 */
bool ASFormatter::isFormattingEnabled() const { return (bracketFormatMode != NONE_MODE || shouldPadOperators || shouldConvertTabs); }
 ...
 * @param mode         the padding mode.
 */
void ASFormatter::setParenthesisPaddingMode(bool state) { shouldPadParenthesies = state; }
 ...
 */
bool ASFormatter::isSequenceReached(const string &sequence) const { return currentLine.COMPARE(charNum, sequence.length(), sequence) == 0; }
...
bool ASFormatter::isBeforeComment() const {
  ...
  for (peekNum = charNum + 1; peekNum < len && isWhiteSpace(currentLine[peekNum]); ++peekNum)
    ;
...
bool ASFormatter::getNextChar() {
  ...
  if (shouldPadOperators && !isInComment && !isInLineComment && !isInQuote && !doesLineStartComment && !isInPreprocessor && !isBeforeComment()) {
    int len = formattedLine.length();
    if (len > 0 && isWhiteSpace(formattedLine[len - 1])) isAfterFormattedWhiteSpace = true;
  }
  ...
  if (!isWhiteSpace(currentChar)) {
    ...
    if (!isInComment && !isInLineComment && !isInQuote && !isSequenceReached(AS_OPEN_COMMENT) && !isSequenceReached(AS_OPEN_LINE_COMMENT))
      previousCommandChar = previousNonWSChar;
  }

  int currentLineLength = currentLine.length();

  if (charNum + 1 < currentLineLength && (!isWhiteSpace(peekNextChar()) || isInComment || isInLineComment)) {
    currentChar = currentLine[++charNum];
    if (isAfterFormattedWhiteSpace)
      while (isWhiteSpace(currentChar) && charNum + 1 < currentLineLength) currentChar = currentLine[++charNum];
...
void ASFormatter::appendSpacePad() {
  int len = formattedLine.length();
  if (len == 0 || !isWhiteSpace(formattedLine[len - 1])) formattedLine.append(1, ' ');
}
...
bool ASFormatter::isUrinaryMinus() const {
  return ((!isalnum(previousCommandChar)) && previousCommandChar != '.' && previousCommandChar != ')' && previousCommandChar != ']');
}
...
bool ASFormatter::isInExponent() const {
  ...
  if (formattedLineLength >= 2) {
    ...
    return ((prevFormattedChar == 'e' || prevFormattedChar == 'E') && (prevPrevFormattedChar == '.' || isdigit(prevPrevFormattedChar)));
  } else
    return false;
}
