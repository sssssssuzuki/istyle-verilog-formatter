...
using std::istream, std::ifstream, std::ofstream, std::ostream, std::cerr, std::cout, std::endl;
using astyle::ASFormatter, astyle::ASStreamIterator;
...
template <class ITER>
bool parseOptions(ASFormatter &formatter, const ITER &optionsBegin, const ITER &optionsEnd, const string &errorInfo) {
  ITER option;
  bool ok = true;
  string arg, subArg;
...
int main(int argc, char *argv[]) {
  ...
  if (shouldParseOptionsFile) {
    ...
    if (optionsFileName.compare("") == 0) {
      ...
      if (path != NULL) optionsFileName = string(drive) + string(path) + string("/.iStylerc");
    }
    ...
    if (!optionsFileName.empty()) {
      ...
      if (optionsIn) {
        ...
        ok = parseOptions(formatter, fileOptionsVector.begin(), fileOptionsVector.end(), string("Unknown option in default options file: "));
      }
  ...
  ok = parseOptions(formatter, optionsVector.begin(), optionsVector.end(), string("Unknown command line option: "));
  if (!ok) {
    printHelpSimple();
    exit(1);
  }
  ...
  if (fileNameVector.empty()) {
    ...
    for (int i = 0; i < fileNameVector.size(); i++) {
      ...
      if (!isWriteable(originalFileName.c_str())) {
        error(string("Error: File '" + originalFileName).c_str(), "' does not exist, or is read-only.");
        continue;
      }
      ...
      if (rename(originalFileName.c_str(), inFileName.c_str()) < 0) {
        error(string("Error: Could not rename " + originalFileName).c_str(), string(" to " + inFileName).c_str());
        exit(1);
      }
